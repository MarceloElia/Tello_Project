...
This repo was built with a deliberately efficient [Claude Code](https://claude.com/claude-code)
setup — the tooling is part of the engineering story, not an afterthought:

- **Repeatable scaffolding.** The installable layout (`src/` package, `pyproject.toml`
  with console-scripts, `tests/`, `docs/`) was generated by a custom Claude Code
  scaffolding skill, so the project started from a consistent, professional Python
  structure instead of being hand-assembled file by file.
- **Codebase knowledge graph.** The whole repo — code, docs and diagrams — is indexed
  into a navigable knowledge graph with [graphify](https://pypi.org/project/graphifyy/)
  (`pip install graphifyy`). It surfaces the core abstractions (`MockTello` and
  `DroneController` come out as the central hubs, exactly as designed), maps the
  cross-module connections, and keeps an EXTRACTED/INFERRED audit trail. On this repo
  it answers architecture questions at roughly **10× lower token cost** than reading
  the files directly. The graph is a regenerable artifact (`graphify-out/`, git-ignored).
